...
class NotificationAccess():
    ...
    def get_notification(self, user_id=None, noti_id=None, is_read=[False, False], get_all=False, as_dict=False):
        ...
        if user_id is not None:
            if is_read[0]:
                noti_obj = self.Notification.query.filter_by(user_id=user_id, isRead=is_read[1]).order_by(
                    self.Notification.created_date.desc())
            else:
                noti_obj = self.Notification.query.filter_by(user_id=user_id).order_by(
                    self.Notification.created_date.desc())
        elif noti_id is not None:
            if is_read[0]:
        ...
        if as_dict and noti_obj is not None:
